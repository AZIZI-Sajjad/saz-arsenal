...
## Protocole permettant de distribuer des adresses IPs à des hôtes
```
AGDLP
```
...
```
Cela permet à des hôtes sur une même réseau d'utiliser une adresse IP virtuelle dans un cadre de haute disponibilité
```
...
## Protocole permettant de créér une connexion sécurisé entre un appareil et un réseau privé via le réseau Internet
```
TPM
```
...
## Protocole qui permets aux administrateurs de se connecter à distance sur des équipements et/ou des machines
```
NSX
```

## Pare feu permettant de mettre en oeuvre la micro-segmentation au niveau des machines virtuelles.
```
Chiffrement
```
...
```
Systeme de fichier pour les systèmes d'exploitations Microsoft
```
...
```
En plus de surveiller le trafic réseau, il permet agir en amont et bloquer la source d'une potentielle attaque
```
...
## Protocole qui gère les autorisations et les authentifications des utilisateurs sur un réseau (exemple portail captif wifi)
```
Redondance
```
